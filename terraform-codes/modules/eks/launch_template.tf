...
resource "aws_launch_template" "eks_nodes" {
  ...
  # Note: IAM instance profile is handled by the EKS node group
  # No need to specify it in the launch template
...
# Use the provided AMI ID - validation is now handled in variables.tf
locals {
  node_ami_id = var.ami_id
}
